...
// Middleware to log raw body for debugging
const rawBody = (req, res, next) => {
  ...
  req.on('end', () => {
    console.log('Raw multipart/form-data body:', data);
    next();
  });
};

// Upload + Transcode
router.post('/upload', authMiddleware, rawBody, upload.single('video'), async (req, res) => {
  console.log('Upload request headers:', JSON.stringify(req.headers, null, 2));
  console.log('Upload request body:', req.body);
  console.log('Upload request file:', req.file);
  if (!req.file) {
  ...
  const jobId = uuidv4();
  const format = req.query.format || 'mov'; // Default to MOV, allow ?format=mov or ?format=avi
  const validFormats = ['mov', 'avi'];
  if (!validFormats.includes(format)) {
    await fs.unlink(req.file.path).catch(() => {});
    return res.status(400).json({ message: `Invalid format. Supported: ${validFormats.join(', ')}` });
  }
  const outputFile = path.join(outputsDir, `${jobId}.${format}`);

  // Store job metadata
  await ensureJobsFile();
  const jobs = JSON.parse(await fs.readFile(jobsFile, 'utf-8'));
  jobs.push({ id: jobId, user: req.user.username, status: 'processing', input: req.file.path, format });
  await fs.writeFile(jobsFile, JSON.stringify(jobs, null, 2));

  // Transcode to specified format (MOV or AVI)
  console.log(`Starting FFmpeg transcoding for job ${jobId} to ${format}`);
  ffmpeg(req.file.path)
    .videoCodec('libx264')
    .audioCodec('aac')
    .format(format)
  ...
  res.json({ message: `Transcoding started to ${format}`, jobId });
});
